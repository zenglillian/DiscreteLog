//Filename: PollardRho_dlp.cpp
//Created by Lillian Zeng
//baby steps giant steps algorithm to solve the discrete log problem in a group
/*This program should be link against gmp library, for example
 g++ PollardRho_dlp.cpp -lgmp -lgmpxx */
    ...
    while(mpz_cmp_ui(d,1)<0|| mpz_cmp(xj,xi)!=0){
        ...
        if(mpz_cmp_ui(d,1)==0)
        {
  	    ...
  	    mpz_div(p1d,p1,d);
            ...
            mpz_mod(l,l,p1d);
   	 mpz_mod(tmodp,t,p);
   while(mpz_cmp(m,d)<0 && mpz_cmp(l,p)<0)
    {
        mpz_add(l,l,p1d);
